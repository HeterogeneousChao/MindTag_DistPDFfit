...
data = record_dat(1,:) - record_dat(2,:);
data = data';
analyse_handles = handles_ana;
...
for indx=1:length(distname)
    try
        ...
        switch dname
            case 'binomial'
                PD=fitbinocase(data); %Special case
            case 'generalized pareto'
                PD=fitgpcase(data); %Special case
            otherwise
                %Built-in distribution using FITDIST
                PD = fitdist(data,dname);
        end
...
%% Plot if requested
if plotind;
    plotfigs(data,D,PD,plotpdf,discind, analyse_handles)
end
...
function plotfigs(data,D,PD,plotpdf,discind,analyse_handles)
